...
$(function(){
	...
	$("#main #logo_box span").hover(function(){
		$("#main #logo_box .more").stop().slideDown(500);
	},function(){
		$("#main #logo_box .more").stop().fadeOut();
	})
	...
	$('.flash ul li').mouseover(function(){//鼠标移入事件
		clearInterval(timer);
		flash_sta = $(this).index();//获得鼠标移入到了第几个li里（鼠标移出后会从当前图片往后播放）
		$('.flash img').eq(flash_sta).fadeIn().siblings('img').hide();//让对应的图片显示，让兄弟图片隐藏
		$('.flash ul li').eq(flash_sta).addClass('cur').siblings('li').removeClass('cur');//控制li的样式
	})
